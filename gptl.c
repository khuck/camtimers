...
/*
** GPTLstartf_virtual: create or start a timer for a specified thread, but do not record any other
**                     information (for use with GPTLstopf_virtual and GPTLrecordf).
**                     As with GPTLstartf, the timer name may not be null terminated.
**
** Input arguments:
**   timername: timer name
**   tid:       specified thread id
**   namelen:   number of characters in timer name
**
** Return value: 0 (success) or GPTLerror (failure)
*/

int GPTLstartf_virtual (const char *timername, int *tid, const int namelen)
  /* timer name and length, and threadid to use for timer*/
{
  Timer *ptr;                 /* linked list pointer */
  int t;                      /* thread index (of this thread) */
  int c;                      /* character index */
  ...
  double tpa = 0.0;           /* time stamp */
  double tpb = 0.0;           /* time stamp */
  bool wall_enabled;          /* local copy of wallstats.enabled */
  bool cpu_enabled;           /* local copy of cpustats.enabled */
  char new_name[MAX_CHARS+1]; /* timer name with prefix, if there is one */
  const char *name;           /* pointer to timer name */
  static const char *thisfunc = "GPTLstartf_virtual";
  ...
  t = *tid;
  if ((t < 0) || (t > maxthreads-1))
    return GPTLerror ("%s: bad thread id\n", thisfunc);
...
**                 GPTLstart(f)_handle, GPTLstartf_virtual, and GPTLrecordf.
**
** Input arguments:
...
**  Called by GPTLstart(f), GPTLstart_instr,
**            GPTLstart(f)_handle, and GPTLstartf_virtual.
**
** Input arguments:
**   ptr:  pointer to timer
**   t:    thread index
...
/*
** GPTLstopf_virtual: stop a timer for a specified thread, but do not update the time or
**                    count information (for use with GPTLstartf_virtual and GPTLrecordf).
**                    As with GPTLstopf, the timer name may not be null terminated.
**
** Input arguments:
**   timername: timer name
**   tid:       specified thread id
...
int GPTLstopf_virtual (const char *timername, int *tid, const int namelen)
  /* timer name and length, and threadid to use for timer*/
{
  Timer *ptr;                 /* linked list pointer */
  int t;                      /* thread number for this process */
  ...
  int numchars;               /* number of characters to copy */
  unsigned int indx;          /* index into hash table */
  long usr = 0;               /* user time (not used - for compatibility with update_stats) */
  long sys = 0;               /* system time (not used - for compatibility with update_stats) */
  char strname[MAX_CHARS+1];  /* null terminated version of name */
  double tpa = 0.0;           /* time stamp */
  double tpb = 0.0;           /* time stamp */
  bool wall_enabled;          /* local copy of wallstats.enabled */
  ...
  char new_name[MAX_CHARS+1]; /* timer name with prefix, if there is one */
  const char *name;           /* pointer to timer name */
  static const char *thisfunc = "GPTLstopf_virtual";
...
/*
** GPTLrecordf: update walltime and call count statistics for a timer. If the timer
**              does not yet exist, create it and initialize with the
**              indicated values.
**
** Input arguments:
**   timername:  timer name
**   tid:        specified thread id
**   count_incr: increment to occurrence count
**   time_incr:  increment to total walltime
**   max_incr:   new max value to compare current max against
**   min_incr:   new min value to compare current min against
**   namelen:    number of characters in timer name
**
** Return value: 0 (success) or -1 (failure)
*/

int GPTLrecordf (const char *timername, int *tid, int *count_incr, double *time_incr,
		 double *max_upd, double *min_upd, const int namelen)
{
  Timer *ptr;                 /* linked list pointer */
  int t;                      /* thread number for this process */
  int c;                      /* character index */
  ...
  char new_name[MAX_CHARS+1]; /* timer name with prefix, if there is one */
  const char *name;           /* pointer to timer name */
  static const char *thisfunc = "GPTLrecordf";
  ...
  */
  ...
  if ( ! ptr) { /* Add a new entry and initialize */
    ...
    */
    ...
    ptr->wall.max = *max_upd;
    ptr->wall.min = *min_upd;
  ...
  ptr->count += *count_incr;
  ptr->wall.accum += *time_incr;

  if (*max_upd > ptr->wall.max)
    ptr->wall.max = *max_upd;

  if (ptr->wall.min == -1)
    ptr->wall.min = *min_upd;
  else if (*min_upd < ptr->wall.min)
    ptr->wall.min = *min_upd;
