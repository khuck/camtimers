...
!-----------------------------------------------------------------------
   ...
   integer,parameter,public :: MPI_ERR_COMM    = 1
#endif
!-----------------------------------------------------------------------
! Private data ---------------------------------------------------------
!-----------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
   ...
   write(pu_logunit,F00) 'WARNING: calling abort() and stopping'
   call shr_sys_flush(pu_logunit)
#if defined(HAVE_MPI)
...
SUBROUTINE mpi_send(buf, count, datatype, dest, tag, communicator, ierr)
...
SUBROUTINE mpi_recv(buf, count, datatype, source, tag, communicator, status, ierr)
